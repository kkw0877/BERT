...
class Warmup(tf.keras.optimizers.schedules.LearningRateSchedule):
    def __init__(self, init_lr, warmup_steps, num_train_steps):
        self.init_lr = init_lr
        self.warmup_steps = warmup_steps
        self.decay_schedule = self._get_decay_schedule(
            init_lr, num_train_steps)

    def __call__(self, step):
        step = tf.cast(step, tf.float32)
        warmup_steps = tf.cast(self.warmup_steps, tf.float32)
        done_percent = step / warmup_steps

        return tf.cond(step < warmup_steps,
                       lambda: self.init_lr * done_percent,
                       lambda: self.decay_schedule(step))

    def _get_decay_schedule(self, init_lr, num_train_steps):
...
def get_optimizer(init_lr, warmup_steps, num_train_steps, beta_1, beta_2):
    lr_schedule = Warmup(init_lr=init_lr,
